...
* 使用 **自建地圖（圖形 Graph）** 與 **Dijkstra 最短路徑演算法**，為使用者規劃至最近適用回收點的導航路線，並在自建的 Canvas 地圖上進行動態路徑繪製。
...
* **設計原理**：
  ...
  * 當導航啟動時，Canvas 上會高亮渲染計算出的最短路徑，引導使用者前往最近的目的地。
* **複雜度**：$O((V + E) \log V)$。
...
2. **自建城市地圖畫布區 (Custom Map Canvas - 中間)**：
   ...
   * **導航操作**：點擊「尋找最近回收點」可自動計算並以高亮流光動畫線條渲染最短路徑。
...
* **流暢的動畫與互動**：
  * Canvas 地圖重繪動作流暢，路徑渲染應具備平滑過渡或高亮效果。
...
1. **手寫資料結構與演算法的真實結合**：透過自建城市地圖，展示手寫的 Graph 鄰接清單與 Dijkstra 演算法在實際回收導航中的應用，完美貼合課程教學。
2. **多智慧體協作系統與通訊佇列**：即時印出帶有時間戳記的 Agent 通訊 JSON 訊息，展示在單頁應用程式中透過 Queue 訊息佇列進行微型非同步通訊的設計。
